...
import (
	...
	"github.com/varsjad/evernest/apps/api/internal/chart"
	...
	"github.com/varsjad/evernest/apps/api/internal/note"
	...
	"github.com/varsjad/evernest/apps/api/internal/sleep"
...
func mountV1(r chi.Router, cfg *config.Config, st *store.Store, logger *slog.Logger) {
	...
	sleepH := sleep.NewHandler(st, logger)
	noteH := note.NewHandler(st, logger)
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/sleeps/{id}", sleepH.ItemRoutes)
		r.Route("/notes/{id}", noteH.ItemRoutes)
	})
}
