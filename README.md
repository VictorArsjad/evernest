...
A baby tracking app — feedings, pumping, diapers, growth, sleep, and charts — designed mobile-first as a PWA, with a household-aware Go backend so multiple caregivers (you, your partner, grandparents) can log activity for the same baby.
...
- Track sleep two ways: tap "Start now" when baby falls asleep (the Sleep tile flips to a live elapsed timer with a "Woke up" button — at most one open session per baby), or enter a past interval manually with fell-asleep/woke-up times. Each entry optionally carries a nap/night tag, a location ("crib", "stroller"…), and notes. Today's total sleep shows as an inline banner stat, and each History day rolls up its total (an overnight sleep counts toward the day it started).
- See sleep *patterns*, not just totals: a wake-window card on Today shows how long baby has been awake against the typical window for their age ("Awake 1h 40m · typical window 1.5–2.5h") with a predicted next-nap time and an overtired zone; the Charts page adds a sleep-rhythm chart (one column per day over 24h, night sleep and naps in different colors, overnight sleeps split at midnight) and a daily-total chart with the age-appropriate recommended range drawn as a band (AAP consensus ranges — not medical advice, and everything hides when the baby has no date of birth or when target comparisons are toggled off in Settings). Night and nap colors are customizable under Settings → Chart colors.
...
- See a combined "Charts & History" view (one shared 7 / 14 / 30 day range control): charts on top — a sparkline per metric (bottle, nursing, pumping, diaper stack) with hover/tap tooltips that surface the active day's value (mouse hover on desktop, tap-to-toggle on mobile); the bottle chart is a stacked bar that splits expressed-breastmilk vs formula per day, and each card's "/day avg" headline excludes today (still in progress) so the average reflects completed days and doesn't drift down as the day ticks forward or reset at midnight (the "total" headline still counts today). Below the charts, History lists every event grouped by local day (Today / Yesterday / weekday), each day prefixed with a one-line roll-up (e.g. "120 ml · 18 min nursing · 3 diapers") — Charts shows daily *totals*, History shows the individual events that made them up. Clicking (desktop) or tapping a bar's tooltip link "View entries →" (touch) jumps straight to that day's History entries, expanding the day and briefly glowing the matching-kind rows so it's obvious which events the bar summarized.
- See a dedicated "Growth" view that plots weight, length, and head circumference against the baby's *age* — not the calendar — on top of the WHO Child Growth Standards percentile curves (3rd / 15th / 50th / 85th / 97th, boys or girls). A plain-language headline surfaces the current percentile ("6.4 kg · 48th percentile · tracking within the typical range"), a metric toggle switches between weight / length / head, and the other two metrics show as at-a-glance percentile chips. Percentiles are computed on the FE from bundled WHO LMS tables (offline-capable, no backend call — the tables are cross-checked against WHO's published per-percentile values; not medical advice). When a baby's sex isn't set, the chart falls back to a trend-only view with a prompt to add it. Reachable from the Today header ("Growth →").
